...
interface ModelInfo {
  ...
  dimension?: number;    // Embedding 模型标识
}
...
export default function ModelSelector({ selectedModelId, onModelChange, disabled }: ModelSelectorProps) {
  ...
  const activeLogoSrc = activeModel ? PROVIDER_LOGOS[activeModel.providerId] : undefined;
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        ...
        <ChevronDown className={`w-3.5 h-3.5 shrink-0 transition-transform text-muted-foreground ${open ? 'rotate-180' : ''}`} strokeWidth={2} aria-hidden="true" />
      </button>
      ...
      {open && (
        ...
        >
          ...
          <div className="overflow-y-auto flex-1">
            ...
            {groups.map((group) => {
              const logoSrc = PROVIDER_LOGOS[group.models[0]?.providerId ?? ''];
              return (
                  ...
                  {group.models.map((model) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* 选中勾 */}
                        {isSelected && (
                          <Check className="w-4 h-4 text-brand shrink-0" strokeWidth={2.5} aria-hidden="true" />
                        )}
                      </button>
                    );
                  })}
