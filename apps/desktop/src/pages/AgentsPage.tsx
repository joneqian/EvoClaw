  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="px-6 pt-5 pb-0 shrink-0">
        <div className="inline-flex bg-accent rounded-xl p-1">
          <button
            ...
            {storeAgents.map((tpl) => (
              ...
              >
                ...
                <div className="flex justify-center mb-4">
                  <div className="w-20 h-20 rounded-full bg-gradient-to-br from-accent to-muted
                    border-2 border-background shadow-sm flex items-center justify-center text-3xl">
                    {tpl.avatar}
                  </div>
                </div>
                {/* 名称 + 描述 */}
                <h4 className="text-base font-bold text-foreground text-center mb-2">{tpl.name}</h4>
                <p className="text-xs text-muted-foreground leading-relaxed text-center flex-1 line-clamp-3">
                  {tpl.desc}
                </p>
                {/* 添加按钮 */}
                <button
                  ...
                  className="mt-4 w-full py-2.5 text-sm font-medium text-muted-foreground
          ...
          <div className="flex-1 overflow-y-auto min-h-0">
            {loading ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center py-16">
                <div className="w-20 h-20 rounded-2xl bg-accent flex items-center justify-center mb-5">
                  <Users className="w-10 h-10 text-muted-foreground" strokeWidth={1} aria-hidden="true" />
                </div>
                <h3 className="text-base font-semibold text-muted-foreground mb-2">还没有专家</h3>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    onClick={handleStartCreate}
                    className="px-5 py-2.5 text-sm font-medium text-muted-foreground
                      bg-card border border-border rounded-xl
                  ...
                  >
                    ...
                    {deleteConfirmId === agent.id ? (
                      <div className="absolute top-1.5 right-1.5 flex gap-1">
                        <button onClick={() => handleDelete(agent.id)}
                          className="px-2 py-0.5 text-xs font-medium text-danger bg-danger/10 rounded">确认</button>
                        <button onClick={() => setDeleteConfirmId(null)}
                          className="px-2 py-0.5 text-xs text-muted-foreground bg-muted rounded">取消</button>
                      </div>
                    ) : (
                      <button onClick={() => setDeleteConfirmId(agent.id)}
                        className="absolute top-2 right-2 w-5 h-5 flex items-center justify-center rounded
                          text-danger hover:text-danger hover:bg-danger/10 transition-colors"
                        title="删除">
                    ...
                    <div className="relative mt-2 mb-2">
                      ...
                      <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-background ${
                        agent.status === 'active' ? 'bg-success' : 'bg-border'
                      }`} />
                    </div>
                    {/* 名称 */}
                    <h4 className="text-sm font-bold text-foreground text-center truncate w-full">{agent.name}</h4>
