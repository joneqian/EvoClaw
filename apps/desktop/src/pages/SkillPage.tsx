...
const SOURCE_LABELS: Record<string, { label: string; color: string }> = {
  ...
  github: { label: 'GitHub', color: 'bg-slate-100 text-slate-700' },
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="px-6 py-4 border-b border-slate-200 bg-white">
        <h2 className="text-lg font-bold text-slate-900 mb-4">Skill 管理</h2>
        ...
        <form onSubmit={handleSearch} className="flex gap-2 mb-3">
          ...
          <button
            ...
            className="px-4 py-2 text-sm font-medium text-white bg-brand rounded-lg hover:bg-brand-active disabled:opacity-50 transition-colors"
          >
            {searching ? '搜索中...' : '搜索'}
          </button>
        </form>
      ...
      {prepareResult && (
        <div className="px-6 py-4 bg-yellow-50 border-b border-yellow-200">
          ...
          <p className="text-xs text-slate-600 mb-2">{prepareResult.metadata.description}</p>
          ...
          {prepareResult.gateResults && prepareResult.gateResults.length > 0 && (
            <div className="mb-2">
              <p className="text-xs font-medium text-slate-700">门控检查:</p>
      ...
      {searchResults.length > 0 && (
        <div className="px-6 py-4 bg-slate-50 border-b border-slate-200">
          <h3 className="text-sm font-medium text-slate-700 mb-2">搜索结果</h3>
          <div className="space-y-2">
            {searchResults.map((r) => {
              const sourceStyle = SOURCE_LABELS[r.source] ?? SOURCE_LABELS.local;
              return (
                <div key={`${r.source}-${r.name}`} className="flex items-center gap-3 p-3 bg-white rounded-lg border border-slate-200">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {r.version && <span className="text-xs text-slate-400">v{r.version}</span>}
                      {r.downloads !== undefined && <span className="text-xs text-slate-400">{r.downloads} 下载</span>}
                    </div>
                  </div>
                  {r.source !== 'local' && (
                    <button
                      ...
                      disabled={installing}
                      className="px-3 py-1.5 text-xs font-medium text-white bg-brand rounded-lg hover:bg-brand-active disabled:opacity-50 transition-colors"
                    >
                      安装
                    </button>
                  )}
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {loading ? (
          ...
          <div className="space-y-2 max-w-3xl mx-auto">
            <p className="text-xs text-slate-400 mb-2">已安装 {skills.length} 个 Skill</p>
            {skills.map((skill) => {
              const sourceStyle = SOURCE_LABELS[skill.source] ?? SOURCE_LABELS.local;
              return (
                <div key={skill.name} className="flex items-center gap-4 p-4 bg-white rounded-lg border border-slate-200 hover:border-brand/40 transition-colors">
                  <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-brand/20 to-brand-active/20 flex items-center justify-center shrink-0">
                    <svg className="w-4 h-4 text-brand-active" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {skill.disableModelInvocation && <span className="px-1.5 py-0.5 rounded text-xs bg-slate-100 text-slate-500">手动触发</span>}
                    </div>
                  </div>
                  <button
