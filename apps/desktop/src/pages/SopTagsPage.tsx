  ...
  return (
    <div className="h-full flex flex-col bg-muted">
      ...
      {confirmModal && (
        ...
        >
          <div
            className="bg-card rounded-xl shadow-xl p-6 w-[400px] max-w-[90vw]"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-base font-semibold text-foreground mb-2">{confirmModal.title}</h3>
            <p className="text-sm text-muted-foreground mb-5 leading-relaxed">{confirmModal.message}</p>
      ...
      {/* 顶部操作栏 */}
      <header className="shrink-0 px-6 py-3 border-b border-border/80 bg-card">
        <div className="flex items-center justify-between mb-2">
          <div>
            <h1 className="text-sm font-semibold text-foreground">SOP 标签设计</h1>
          ...
          <button
            ...
            className="px-3 py-1 text-xs text-muted-foreground border border-border rounded-lg hover:bg-muted transition-colors"
          >
            刷新
          </button>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {generating ? (
              <>
                <span className="inline-block w-3 h-3 border-2 border-background/40 border-t-white rounded-full animate-spin" />
                生成中…
              </>
            ) : (
              <>🤖 AI 生成草稿</>
      ...
      <div className="flex-1 min-h-0 flex">
        {/* ─── 左栏：文档管理 ─── */}
        <aside className="w-[280px] shrink-0 border-r border-border/80 bg-card flex flex-col">
          <div className="p-4 border-b border-border">
            <h2 className="text-sm font-semibold text-foreground mb-2">参考文档</h2>
            ...
            <div
              ...
              className="border-2 border-dashed border-border rounded-lg p-4 text-center cursor-pointer hover:border-brand/50 hover:bg-brand/5 transition-colors"
            >
          ...
          <div className="flex-1 overflow-y-auto">
            {docs.length === 0 ? (
              ...
              <ul className="divide-y divide-border">
                {docs.map((doc) => (
                  <li key={doc.id} className="px-4 py-3 hover:bg-muted group">
                    <div className="flex items-start gap-2">
                      <span className="shrink-0 inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-accent text-muted-foreground">
                        {doc.ext}
                      </span>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-foreground truncate" title={doc.originalName}>
                          {doc.originalName}
                        </p>
                      ...
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 text-[10px] text-danger hover:text-danger transition-opacity"
                      >
                        删除
                      </button>
                    </div>
        ...
        <main className="flex-1 min-w-0 flex flex-col">
          {/* Tab */}
          <div className="px-6 pt-4 flex items-center gap-1 bg-card border-b border-border/80">
            <button
            ...
            >
              草稿（待审核）{draft && draft.length > 0 ? <span className="ml-1 px-1.5 py-0.5 text-[10px] bg-warning/15 text-warning rounded-full">{draft.length}</span> : ''}
            </button>
            <button
            ...
            >
              已确认 {tags.length > 0 ? <span className="ml-1 px-1.5 py-0.5 text-[10px] bg-success/15 text-success rounded-full">{tags.length}</span> : ''}
            </button>
          </div>
          ...
          {(errMsg || okMsg) && (
            <div className={`mx-6 mt-3 px-3 py-2 rounded-lg text-xs ${
              errMsg ? 'bg-danger/10 text-danger' : 'bg-success/10 text-success'
            }`}>
              {errMsg ?? okMsg}
            </div>
          )}
          ...
          {/* 底部操作栏（仅 draft tab） */}
          {tab === 'draft' && (
            <div className="shrink-0 border-t border-border/80 bg-card px-6 py-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={!draft}
                  className="px-3 py-1.5 text-xs text-muted-foreground border border-border rounded-lg hover:bg-muted disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
                  丢弃草稿
                </button>
                <button
  ...
  return (
    <div className="space-y-4 max-w-4xl">
      {editing.map((parent, pi) => (
        <div key={pi} className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
          {/* 父标签头 */}
          <div className="flex items-center gap-2 px-4 py-3 bg-muted/60 border-b border-border">
            <span className="text-xs font-semibold text-muted-foreground">阶段 {pi + 1}</span>
            <input
              value={parent.name}
          ...
          <div className="divide-y divide-border">
            {parent.children.map((child, ci) => (
              <div key={ci} className="px-4 py-3 grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-medium text-muted-foreground mb-1">标签名称</label>
                  <input
                    value={child.name}
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    <label className="block text-[10px] font-medium text-muted-foreground mb-1">标签含义</label>
                    <input
                      value={child.meaning}
                  ...
                  <button
                    ...
                    className="text-[10px] text-danger hover:text-danger mt-5"
                  >
                    删除
                  </button>
                </div>
                <div className="col-span-2">
                  <label className="block text-[10px] font-medium text-success mb-1">需要做（必须说什么 / 做什么）</label>
                  <textarea
                    value={child.mustDo}
                ...
                <div className="col-span-2">
                  <label className="block text-[10px] font-medium text-danger mb-1">不能做（禁止说什么 / 做什么）</label>
                  <textarea
      ...
      <button
        onClick={onAddParent}
        className="w-full py-3 text-xs text-muted-foreground border-2 border-dashed border-border rounded-xl hover:border-brand/50 hover:text-brand transition-colors"
      >
        + 添加父标签（阶段）
      </button>
    </div>
      ...
      {tags.map((parent, pi) => (
        <div key={pi} className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
          <div className="px-4 py-3 bg-muted/60 border-b border-border">
            <span className="text-xs font-semibold text-muted-foreground mr-2">阶段 {pi + 1}</span>
            <span className="text-sm font-semibold text-foreground">{parent.name}</span>
          </div>
          <div className="divide-y divide-border">
            {parent.children.map((child, ci) => (
              <div key={ci} className="px-4 py-3">
                <div className="text-sm font-medium text-foreground mb-1">{child.name}</div>
                <div className="text-xs text-muted-foreground mb-2">{child.meaning}</div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="text-xs">
                    <div className="font-medium text-success mb-0.5">需要做</div>
                    <div className="text-foreground whitespace-pre-line">{child.mustDo}</div>
                  </div>
                  <div className="text-xs">
                    <div className="font-medium text-danger mb-0.5">不能做</div>
                    <div className="text-foreground whitespace-pre-line">{child.mustNotDo}</div>
                  </div>
                </div>
              </div>
            ))}
