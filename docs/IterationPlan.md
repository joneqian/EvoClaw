...
**当前进度**: Sprint 1 ✅ | Sprint 2 ✅ | Sprint 3 待开始
...
## Sprint 6: 本地知识库 RAG + 向量持久化（Week 11-12）
...
#### 6.1 向量存储持久化升级
- [ ] 评估 sqlite-vec 稳定性（检查最新版本、issue 列表、breaking changes）
  - 若稳定 → 安装 `sqlite-vec` 并替换 `VectorStore` 内部实现为 sqlite 虚拟表
  - 若仍不稳定 → 使用 `usearch` 或 `hnswlib-node` 持久化到本地文件
- [ ] 实现 Embedding API 调用层 `src/rag/embedding-provider.ts`：
  - `generateEmbedding(text: string): Promise<Float32Array>` — 调用 LLM embedding API
  - 支持 OpenAI `text-embedding-3-small`（1536 维）和国产 embedding（通义/智谱）
  - 批量接口 `generateEmbeddings(texts: string[]): Promise<Float32Array[]>`
  - 通过 ModelRouter 选择 embedding provider
- [ ] 升级 `src/infrastructure/db/vector-store.ts`：
  - 替换内存 Map 为持久化存储（接口不变，调用方零改动）
  - 添加 `persistToDisk()` / `loadFromDisk()` 生命周期（若用非 sqlite 方案）
- [ ] 升级 `src/memory/memory-store.ts`：insert/update 时同步生成 embedding 并写入向量索引
- [ ] 升级 `src/memory/hybrid-searcher.ts`：启用 Phase 1 向量搜索路径（权重 0.5）
- [ ] 编写向量持久化集成测试
...
- [ ] 创建 `migrations/009_knowledge_base.sql`：
  ...
  - `knowledge_base_chunks` 表（id, file_id, content, metadata_json, embedding）

#### 6.4 RAG 检索插件
- [ ] 实现 `src/context/plugins/rag.ts`（priority: 50）：
...
#### 6.5 知识库管理 UI
- [ ] 实现知识库管理页面（`apps/desktop/src/pages/KnowledgePage.tsx`）：
  - 已索引文件列表（状态、大小、块数）
  - 拖拽/选择文件导入
  - 删除/重新索引操作
  - 索引进度条
...
#### 6.6 Sprint 6 验收
- [ ] **向量存储已从内存 fallback 升级为持久化方案**（进程重启后向量不丢失）
- [ ] **记忆系统 hybrid search 的向量搜索路径已启用**（权重 0.5 生效）
