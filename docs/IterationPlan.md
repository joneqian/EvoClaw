...
#### 15.1 子 Agent 派生
- [x] 实现 `src/agent/sub-agent-spawner.ts`：_(Sprint 10C Day 2 已完成)_
  - `spawn(parentAgentId, config)` → 创建子 Agent 会话
    - 继承父 Agent 的 AGENTS.md + TOOLS.md（不继承 USER.md/MEMORY.md）_(Sprint 10C Day 2 追加)_
    - 独立的 Session + 独立的 JSONL 会话记录
    - 指定任务 prompt + 约束条件
  - `getSubAgentStatus(subAgentId)` → 查询状态
  - ~~`sendToSubAgent(subAgentId, message)` → 向子 Agent 发消息~~（改为 kill 终止）
  - ~~`getSubAgentHistory(subAgentId)` → 获取子 Agent 对话记录~~（子 Agent 完成后回传结果）
  - `terminateSubAgent(subAgentId)` → 终止
- [x] 实现安全约束：_(Sprint 10C Day 2 已完成)_
  - 派生深度限制：`MAX_SPAWN_DEPTH` 默认 2（最多 2 层嵌套）
  - 并发限制：subagent 车道 8 并发（LaneQueue）
  - 子 Agent 超时：120s
  - 子 Agent 不继承父 Agent 的私密记忆（USER.md/MEMORY.md 不继承）
- [x] 注册工具：_(Sprint 10C Day 2 已完成，用工具名替代 PI sessions_ 前缀)_
  - `spawn_agent` → 创建子 Agent
  - `list_agents` → 列出活跃子 Agent
  - `kill_agent` → 终止子 Agent
- [x] 编写子 Agent 测试（`src/__tests__/sub-agent-spawner.test.ts`）_(Sprint 10C Day 2)_
