...
**当前进度**: Sprint 1 ✅ | Sprint 2 ✅ | Sprint 3 ✅ | Sprint 4 ✅ | Sprint 5 ✅ | Sprint 6 ✅
...
- [ ] 实现 `src/skill/skill-gate.ts`：门控检查（EvoClaw 自定义扩展，PI/AgentSkills 规范本身不实现门控）
  ...
  - 不满足 → 提示安装缺失依赖或静默跳过
  - 注意：`compatibility` 在 AgentSkills 规范中是纯信息性字段（max 500 chars），EvoClaw 额外解析其中的依赖信息
...
- [ ] 实现 `src/context/plugins/tool-registry.ts`（priority: 60）：
  - beforeTurn：扫描已安装 Skill → 生成 `<available_skills>` XML 目录注入 system prompt
  - 遵循 PI 渐进式注入模式：Tier 1 目录注入（name + description + location，~50-100 tokens/skill），Tier 2 模型用 Read 工具按需加载完整 SKILL.md
  - Skill 不注册新工具 — 通过指令引导模型使用已有工具（Read/Bash/Write）
  - `allowed-tools` 字段做权限预批准（实验性）
  - Skill 加载优先级：Agent 工作区 > 用户级安装 > 内置
  - 扫描路径：`~/.evoclaw/agents/{id}/workspace/skills/` + `~/.evoclaw/skills/`
  - 根目录 `.md` 文件直接作为 Skill，子目录中只识别 `SKILL.md`
  - `disable-model-invocation: true` 的 Skill 不出现在目录中（仅 `/skill:name` 可触发）
  - EvoClaw 自定义门控检查（extends PI，PI 本身不实现门控）：不满足条件的 Skill 静默跳过
...
#### 7.6 Skill 管理 UI
- [ ] 实现 Skill 管理页面（`apps/desktop/src/pages/SkillPage.tsx`）：
...
#### 7.7 Sprint 7 验收
- [ ] 可通过 ClawHub API 搜索 Skill（向量语义搜索）
- [ ] 支持 GitHub URL 直装 Skill（兼容 skills.sh 生态）
- [ ] Skill 安装流程完整（prepare: 下载 → 分析 → 门控 → 展示报告 → confirm: 安装）
