...
| **F3.3 记忆提取 Pipeline** | 三阶段流水线：预处理 → LLM 提取 → 持久化，含反馈循环防护和文本清洗 | **P0** |
...
| **F4.2 多源搜索** | 从多个来源搜索匹配的 Skill/MCP：ClawHub (13,700+ Skills)、skills.sh、工作区 Skills。搜索结果包含评分、下载量、安全评级。遵循 AgentSkills 规范（SKILL.md 格式） | **P1** |
...
| **F4.6 Skill 门控机制** | 安装和加载 Skill 前自动检查门控条件：requires.bins（PATH 中需要的二进制文件）、requires.env（需要的环境变量）、requires.os（平台限制）。不满足条件的 Skill 静默跳过或提示安装缺失依赖 | **P1** |

#### 验收标准

- [ ] Agent 执行任务失败时，80% 以上的情况能正确识别是否为能力缺口
- [ ] 搜索结果在 3 秒内返回，至少覆盖 ClawHub 和 skills.sh 两个来源
...
| **Skill 生态可用** | 假设可以直接复用 ClawHub (13,700+ Skills) 和 skills.sh 生态的已有 Skill |
...
| **Skill 自发现** | 能力缺口检测 + 安全安装流（来源：ClawHub + skills.sh） |
...
| W9 | Skill 自发现 + 安全安装流（ClawHub + skills.sh） + 飞书/企微 Channel（含 Binding 路由 + Session Key 路由） |
| W10 | macOS 应用打包 + 内测发布 |
...
- Skill 自发现与安全安装（ClawHub + skills.sh）
- 飞书 + 企微 Channel 接入
...
| **Skill 自发现** | 能力缺口检测 + 自动搜索安装（ClawHub + skills.sh） | 手动搜索安装 | 手动配置 | 不支持 | 有限 |
...
| R5 | **Skill 生态可用性** -- 自发现机制搜到的 Skill 不适配或质量差 | 生态 | 低 | 中 | 直接复用 ClawHub 13,700+ Skills，已经过社区验证；Skill 门控机制（requires.bins/env/os）自动过滤不兼容的 Skill；安全安装流（签名验证 + 沙箱试运行）保障质量 |
...
> **文档版本**: v4.0 -- 基于 PI 框架集成、L0/L1/L2 三层记忆系统、ContextPlugin 架构、ClawHub 生态兼容的重大升级
> **文档状态**: 已更新
> **下次评审**: 待定
