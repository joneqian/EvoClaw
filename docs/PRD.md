...
> **核心理念**: Agent 的能力取决于它能使用的工具 -- 通过内置高质量工具集，让 Agent 从"只会聊天"进化为"能做事的助手"。

#### 用户故事

> 小王对 EvoClaw 说："帮我搜一下 Rust 异步运行时的最新进展"，Agent 自动调用 Web 搜索工具获取最新信息并汇总。接着他发了一个 PDF 链接："帮我读一下这份技术报告"，Agent 抓取网页内容并提取关键信息。最后他拖入一张架构图："分析一下这个系统设计有什么问题"，Agent 通过图片分析工具给出了专业建议。

#### 功能拆解

| 子功能 | 描述 | 优先级 |
|--------|------|--------|
| **F9.1 Web 搜索工具** | 内置 web_search 工具，支持 Brave Search API，Agent 可主动搜索互联网获取实时信息。API Key 通过 Provider 机制统一管理 | **P0** |
...
| **F9.5 apply_patch 工具** | 多文件统一 diff 格式，比逐文件 edit 更高效的代码修改方式。兼容 OpenClaw 的 patch 格式 | **P1** |
| **F9.6 后台进程管理** | exec 工具支持后台执行，process 工具管理长运行命令（轮询输出、发送按键、终止） | **P1** |
| **F9.7 系统提示增强** | 模块化系统提示构建：安全宪法 + 记忆召回指令 + 运行时信息 + 工具使用指导 + 技能扫描指令。参考 OpenClaw 的 22 段式系统提示架构 | **P0** |
...
> **核心理念**: 一个会崩溃的助手不如没有助手 -- 多级错误恢复让 Agent 在各种异常下都能优雅处理。
...
| 子功能 | 描述 | 优先级 |
|--------|------|--------|
| **F10.1 多级错误恢复** | Auth 轮转 → overload 指数退避 → thinking 级别降级 → context overflow 自动 compaction，参考 OpenClaw 的 160 次重试策略 | **P0** |
| **F10.2 工具循环检测** | 检测 Agent 工具调用的重复模式、乒乓模式，全局熔断器（阈值 30 次）自动终止 | **P0** |
| **F10.3 工具结果截断** | 自动截断超大工具输出（代码文件、日志等），防止 context window 溢出 | **P0** |
...
| **Agent 增强工具集** | Web 搜索（F9.1）+ Web 抓取（F9.2）+ 图片分析（F9.3）+ PDF 阅读（F9.4）+ 系统提示增强（F9.7） |
| **Agent 可靠性保障** | 多级错误恢复（F10.1）+ 工具循环检测（F10.2）+ 工具结果截断（F10.3） |
...
> **文档版本**: v5.0 -- 新增 Agent 增强工具集（F9.1-F9.7）和 Agent 可靠性保障（F10.1-F10.6），基于 OpenClaw 研究成果；包含 Web 搜索/抓取、图片分析、PDF 阅读、apply_patch、多级错误恢复、工具循环检测等特性
> **文档状态**: 已更新
> **下次评审**: 待定
