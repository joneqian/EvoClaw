...
- [ ] 端到端测试场景：
  ...
  - [x] 记忆沉淀：对话后 → 记忆提取 → memory_units 写入 _(Sprint 10C Day 2 完成：memoryExtractPlugin afterTurn 接入)_
  - [x] 记忆召回：下次对话 → 三阶段检索 → 记忆注入上下文 _(Sprint 10C Day 2 完成：memoryRecallPlugin beforeTurn 接入)_
  - [ ] 长对话压缩：10+ 轮对话 → LCM 触发 → 摘要生成 → 上下文精简
  - [ ] Hotness 衰减：模拟时间推移 → activation 下降 → 归档
  - [x] 反馈循环防护：注入记忆 → 提取不含注入内容 _(Sprint 4 已实现零宽空格标记，Sprint 10C Day 2 管线接通后生效)_
...
| 工具循环检测 + 结果截断 | ✅ 完成 | 03-19 |
| 后台进程管理（exec 后台 + process 工具） | ✅ 完成 | 03-19 |
| Doctor 自诊断命令 | ✅ 完成 | 03-19 |
| 端到端测试 + Bug 修复 + 演示准备 | ✅ 完成 | 03-19 |
| 记忆系统接入聊天流（recall/extract 插件 + 管线 Bug 修复） | ✅ 完成 | 03-18 |
| 工作区文件系统完善（USER.md 渲染 + MEMORY.md 指令 + BOOTSTRAP.md 跳过 + 子 Agent 继承 + 文件截断） | ✅ 完成 | 03-18 |
...
### Day 2（03-18）— 专业工具 + 子 Agent + 记忆打通 + 工作区完善 ✅

**目标**: 让 Agent "能读图、能读 PDF、能改代码、能分身、有记忆、懂用户"
...
| 记忆系统接入聊天流 | 2h | recall/extract 插件注册 + 管线 Bug 修复（Embedding baseUrl、LLM 协议、中文分词、FTS OR、XML 解析） | ✅ |
| 工作区文件系统完善 | 1h | USER.md 动态渲染 + MEMORY.md 读写指令 + BOOTSTRAP.md 首轮即弃 + 子 Agent 继承 AGENTS.md/TOOLS.md + 文件截断保护(20K/150K) | ✅ |
...
- **记忆管线打通** — `memoryRecallPlugin`(beforeTurn) + `memoryExtractPlugin`(afterTurn) 注册到 ContextEngine；`HybridSearcher` + `MemoryExtractor` 从 server.ts 透传到 chat 路由
- **管线 Bug 修复 8 项** — MemoryExtractor 缺 VectorStore/FtsStore 索引、Embedding baseUrl /v1 重复、callLLM 'anthropic' 协议匹配错误、中文分词停用词+单字切分、FTS AND→OR、XML 解析器 fallback（混合内容+代码块）、提取 prompt XML-only 约束、全链路日志
- **工作区文件系统 5 项完善** — MEMORY.md 读写指令+`<agent_notes>`注入、USER.md `UserMdRenderer`动态渲染+`<user_profile>`注入、BOOTSTRAP.md 仅首轮注入、子 Agent 继承 AGENTS.md+TOOLS.md、单文件 20K/总量 150K 截断保护
- 测试 760 全通过，esbuild 打包成功
...
| Doctor 自诊断 | 1h | 至少 12 项检查 | ✅ |
| 端到端测试 + Bug 修复 | 3h | 77 文件 760 测试全通过 + 构建成功 | ✅ |

**完成摘要**:
- `tool-safety.ts` — ToolSafetyGuard 类：重复模式检测（同工具+参数连续 N 次）、乒乓模式检测（两工具交替）、全局熔断（阈值 30）、结果截断（50K 字符），集成到 embedded-runner 的工具 execute 包装层
- `background-process.ts` — ProcessManager 类 + exec_background/process 两个工具：后台启动命令、环形输出缓冲(200 行)、list/output/kill/send 操作、危险命令检测（rm -rf、fork bomb 等）
- `doctor.ts` — 12 项诊断检查 + GET /doctor 路由：Node.js 版本、数据库连接/表完整性、配置文件/Provider/默认模型/Embedding 模型、磁盘空间、内存使用、LaneQueue 状态、PI 框架可用性、Agent 数量，返回 healthy/degraded/unhealthy 总体状态
- chat.ts 注入 exec_background + process 工具
- server.ts 挂载 /doctor 路由
- 测试 760 全通过（新增 36），esbuild 打包成功
...
#### 15.1 子 Agent 派生（Sprint 10C 提前完成，参考 OpenClaw subagent-spawn.ts）
- [x] 实现 `src/agent/sub-agent-spawner.ts`：_(Sprint 10C Day 2)_
  - `spawn(task, context, timeout, {agentId, attachments})` → 创建子 Agent 会话
    - 继承目标 Agent 的 AGENTS.md + TOOLS.md（不继承 USER.md/MEMORY.md）
    - 独立 Session + 独立系统提示
  - `list()` → 列出所有子 Agent 状态和结果
  - `kill(taskId)` → 终止子 Agent
  - `steer(taskId, correction)` → 纠偏运行中的子 Agent（终止+纠正指令重启）_(Sprint 10C 追加)_
  - `collectCompletedResults()` → 推送式获取未读结果 _(Sprint 10C 追加)_
- [x] 实现安全约束：_(Sprint 10C Day 2 + 追加)_
  - 三层角色：main(depth=0) / orchestrator(1..max-1) / leaf(>=max)
  - 派生深度限制：`MAX_SPAWN_DEPTH` 默认 2
  - 每 Agent 子代上限：`MAX_CHILDREN_PER_AGENT` 默认 5
  - 并发限制：subagent 车道 8 并发（LaneQueue）
  - 子 Agent 超时：默认 300s，可配置（10-3600s）
  - 工具降权：子 Agent 不获得 memory_search/memory_get/knowledge_query；leaf 额外不获得 spawn/list/kill/steer/yield
  - 不可信结果标记：`<<<BEGIN_UNTRUSTED_CHILD_RESULT>>>` 包裹防提示注入
- [x] 注册工具（5 个）：_(Sprint 10C)_
  - `spawn_agent` → 创建子 Agent（支持 timeout/agentId/attachments 参数）
  - `list_agents` → 列出子 Agent 状态和完整结果
  - `kill_agent` → 终止子 Agent
  - `steer_agent` → 纠偏运行中的子 Agent
  - `yield_agents` → 让出当前轮次等待子 Agent 结果
- [x] 跨 Agent 生成：_(Sprint 10C 追加)_
  - `AgentResolver` 回调解析目标 Agent 配置 + 工作区文件
  - `allowAgents` 白名单控制（undefined=全部允许，[]=禁止）
  - 跨 Agent 时加载目标 Agent 的 SOUL.md/IDENTITY.md/AGENTS.md/TOOLS.md
- [x] 附件传递：_(Sprint 10C 追加)_
  - `spawn_agent` 支持 `attachments: [{name, content}]` 参数
  - 附件作为 `<attachments>` 段注入子 Agent 系统提示
- [x] 编写子 Agent 测试（`src/__tests__/sub-agent-tools.test.ts`）

#### 15.1b 子 Agent 进阶（P3 — 从 OpenClaw 研究中识别的后续项）
- [ ] 持久会话模式（`mode: "session"`）：
  - 子 Agent 保持长期会话状态，可多次交互
  - 会话存储到 conversation_log，支持恢复
  - `sessions_send` 工具向持久子 Agent 发送后续消息
- [ ] Nested 车道（内联 Agent 调用）：
  - 新增 `CommandLane.Nested` 用于同步内联 Agent 调用
  - 区别于 subagent 车道（异步后台），nested 车道用于需要立即结果的场景
  - 评估 EvoClaw 是否有此需求后决定是否实现
...
#### 15.6 Sprint 15 验收
- [x] 父 Agent 可成功派生子 Agent 执行任务 _(Sprint 10C 已完成)_
- [x] 子 Agent 结果正确回传给父 Agent _(Sprint 10C 已完成)_
- [x] 派生深度限制生效（超过 maxSpawnDepth 拒绝派生）_(Sprint 10C 已完成)_
- [x] 并发限制生效 _(Sprint 10C 已完成：车道 8 并发 + 每 Agent 5 子代)_
- [ ] Agent 间通信默认关闭，显式开启后正常工作
- [x] 子 Agent 不继承父 Agent 的私密记忆 _(Sprint 10C 已完成：工具降权 + 不继承 USER.md/MEMORY.md)_
- [ ] 用户空闲时后台任务自动执行，活跃时暂停
