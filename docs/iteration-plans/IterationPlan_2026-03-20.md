...
| 15.12.4 | **DB 迁移 025_memory_feedback.sql**：新表 `memory_feedback (id, memory_id FK, agent_id, type CHECK IN ('inaccurate'\|'sensitive'\|'outdated'), note, reported_at, resolved_at)`，并在 `memory_units` 表字段补 `confidence` 降权写入路径 | P0 | 0.5d | ✅ | F3.22 |
| 15.12.5 | **记忆中心后端 API 补齐**：`routes/memory.ts` 新增 `PUT /:agentId/units/:id`（更新 L1/L2，L0 锁死）、`POST /:agentId/units/:id/feedback`（写 memory_feedback，所有 type 自动 confidence -= 0.15，下限 0）、`GET /:agentId/knowledge-graph?limit=...`（实体关系三元组分页）、`GET /:agentId/consolidations?limit=...`（AutoDream 历史）、`GET /:agentId/session-summaries?limit=...` | P0 | 1d | ✅ | F3.22 |
| 15.12.6 | **召回元数据回传**：修改 `memory-recall.ts` 把 `results.map(r => r.memoryId)` 写入 `ctx.recallMeta?: { memoryIds: string[]; scores: number[]; l0Indexes: string[]; categories: string[] }`（新增 `TurnContext` 字段），由 chat.ts 在 SSE 流末尾通过新事件 `recall_meta` 透传给前端 | P0 | 1d | ✅ | F3.23 |
...
| 15.12.9 | **前端记忆中心：知识图谱视图**：`MemoryPage.tsx` 顶部 Tab 新增"知识图谱"，**实际实现**：表格视图按 subject 分组（不引入 force graph 库，节省 ~100KB bundle） | P1 | 1d | ✅ | F3.22 |
...
| 15.12.12 | **集成测试**：`memory-feedback-store.test.ts`（16 个 store CRUD 测试）+ `e2e-memory-feedback.test.ts`（18 个端点 e2e 覆盖 5 个新端点 + 跨 agent 隔离 + limit 分页）+ `e2e-memory-full-loop.test.ts`（3 个全链路测试: write → query → flag → 多次衰减 → 下限 0 + 编辑后再反馈 + 级联删除）+ Phase A 已有的 evoclaw-tools.test.ts 30 个工具单元测试 + builtin-commands.test.ts 8 个 slash 命令测试 | P0 | 1d | ✅ | — |
| 15.12.13 | **E2E + 文档更新**：完整 e2e 链路覆盖（write → query → flag → decay → 下限 → 级联）；`MemorySystemDesign.md` 加第 15 节"用户触达层（Sprint 15.12）"，描述 5 Phase 闭环 + 设计决策 | P1 | 0.5d | ✅ | — |
...
**总工作量**: 计划 ~10.5d，**实际 1 天**（2026-04-09 完成）

**实际交付物**:
- 修改文件: `tools/evoclaw-tools.ts`（+5 工具 + skipWebTools 标志）, `agent/embedded-runner-prompt.ts`（TOOL_SUMMARIES + memory_recall 段引导）, `routes/memory.ts`（+5 端点）, `context/plugins/memory-recall.ts`（写 recallMeta 含 l0Index/category）, `context/plugin.interface.ts`（TurnContext 加 recallMeta 字段）, `routes/chat.ts`（接通 createEvoClawTools + SSE recall_meta + 修复 turnCtx 类型）, `agent/agent-manager.ts`（initWorkspace 退役 mkdir memory/ + AGENTS.md 模板 + BOOTSTRAP.md 模板 + 注册 slash 命令）, `agent/agent-builder.ts`（BOOTSTRAP.md 模板）, `agent/memory-flush.ts`（**完整重构** 改用 memory_write，删 8 个废弃 helper）, `agent/kernel/context-compactor.ts`（删读日记指令）, `agent/tool-catalog.ts`（CORE_TOOLS + TOOL_PROFILES 注册 5 个新工具）, `context/plugins/tool-registry.ts`（Skill prompt header 改 optional reference library）, `tools/permission-interceptor.ts`（AUTO_ALLOW_TOOLS 加 8 个 memory 工具）, `server.ts`（**关键修复 memorySys return 漏 knowledgeGraph** + CreateAppOptions + createApp 调用 + createChatRoutes 调用）, `routes/channel-message-handler.ts`（同样接通）, `apps/desktop/src/pages/MemoryPage.tsx`（EditDialog + FeedbackDialog + StalenessBadge + ConfidenceDot/HotnessDot + 4 Tab 切换 + 3 个新 Tab 组件）, `apps/desktop/src/stores/memory-store.ts`（updateMemory/flagMemory + 3 个 Phase D fetch actions + KnowledgeRelation/ConsolidationRun/SessionSummary 类型）, `apps/desktop/src/pages/ChatPage.tsx`（RecallMetaBar + recall_meta SSE 处理）, `apps/desktop/src/stores/chat-store.ts`（Message.recallMeta + setLastMessageRecallMeta action）
- 新增文件: `packages/core/src/infrastructure/db/migrations/025_memory_feedback.sql`（迁移号 025，021 已被占用）, `packages/core/src/channel/command/builtin/remember.ts`, `packages/core/src/channel/command/builtin/forget.ts`, `packages/core/src/memory/memory-feedback-store.ts`
- 测试新增: `__tests__/memory-feedback-store.test.ts` (16 tests), `__tests__/e2e-memory-feedback.test.ts` (18 tests), `__tests__/e2e-memory-full-loop.test.ts` (3 tests)。Phase A 同时在 `evoclaw-tools.test.ts` (+30) 和 `channel/builtin-commands.test.ts` (+8) 加测试。Phase A 重构 `memory-flush.test.ts` (-11 老测试 +14 新测试)。**净增 78 个测试**（2347 → 2414 全部通过）
- 文档: `docs/architecture/MemorySystemDesign.md` 加第 15 节"用户触达层"

**验收标准**:
- [x] 用户对 Agent 说"记住我女儿叫小满，5 月 3 日生日" → Agent 同轮返回"已记住（id=mem_xxx）" → 立即在记忆中心看到该条 → **不依赖 afterTurn 异步抽取** ✅ 实测通过（Claude Sonnet 4.6，1 turn 调 memory_write）
- [x] 用户对 Agent 说"忘掉所有关于客户 ABC 的事" → memory_forget_topic 工具实现 ✅ 测试通过
- [x] 在记忆中心点"编辑"按钮可以改 L1/L2 内容并保存（L0 字段灰显不可改）✅
- [x] 在记忆中心点"不准确"按钮 → 选类型/写备注 → 提交后该记忆 confidence -0.15 ✅
- [x] 列表项 >1 天显示 `1d+` 黄标、>7 天显示红标 ✅
- [x] "知识图谱" Tab 渲染当前 Agent 的实体关系（表格视图按 subject 分组）✅
- [x] "整理历史" Tab 时间线展示 AutoDream 运行 ✅
- [x] "会话摘要" Tab 列出 session_key 摘要 + react-markdown 渲染 ✅
- [x] 聊天页 Agent 回复上方出现"本轮用到 N 条记忆"折叠条 + "不准"按钮 ✅ 实测通过
- [x] `/remember` 和 `/forget` slash 命令兜底可用 ✅
- [x] 全量测试通过，无回归 ✅ 2414 tests pass
- [x] e2e 完整链路（说→看→改→标→降权）跑通 ✅ e2e-memory-full-loop.test.ts

**Sprint 15.12 提交**:
- `0e541ed` Phase A — DB-first memory tools + retire diary path
- `dd2109f` Phase B — feedback schema + 5 new memory endpoints
- `fdcf1f0` Phase C — recall metadata SSE + frontend edit/feedback/staleness
- `d2bf944` Phase C UX fixes — refetch after write + clarify confidence vs hotness labels
- `15b415a` Phase D — knowledge graph + consolidations + session summaries tabs
- `1bc5069` Phase E — Show Your Work fold-bar in ChatPage
- *(Phase F commit pending)* Phase F — full-loop e2e + design doc + iteration plan
