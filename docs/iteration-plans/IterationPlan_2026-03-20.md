...
> **文档版本**: v6.2
> **创建日期**: 2026-03-20
> **文档状态**: 执行中（Sprint 11 ✅ 已完成, Sprint 12 ✅ 已完成, Sprint 13 ✅ 已完成, Sprint 14 🔨 进行中）
> **基于**: PRD v6.2 + Architecture v6.2 + EvoClaw vs OpenClaw 对比报告 (2026-03-20)
...
#### Sprint 14: 工具系统优化（W5-W8，约 4 周）🔨 进行中
...
**验收标准**:
- [ ] MCP server 可通过 evo_claw.json 配置并正常调用，工具自动出现在 Agent 工具列表
- [ ] Read 大文件（>10K 行）自动分页，无需用户手动指定 offset/limit
- [ ] Gemini 模型工具调用不报 schema 兼容性错误（additionalProperties/anyOf 等）
- [ ] Memory Flush 在 context 使用率达 80% 前正确触发，记忆持久化到 memory_units 表
- [ ] safeBins 白名单外的命令执行触发权限审批流程
- [ ] 子代理禁止列表可在 Agent 配置中显式管理
- [ ] 工具目录元数据可通过 API 查询（`GET /tools/catalog`）

---

#### Sprint 15: 企微 Channel 生产就绪（W9-W10）

> **注**: 原 Sprint 13 企微生产化任务顺延至 Sprint 15，原 Sprint 15 飞书生产化顺延至 Sprint 16。

**目标**: 企微 Channel 达到生产级别。

| # | 任务 | 优先级 | 预估 | 对应 Feature |
|---|------|--------|------|-------------|
| 15.1 | 企微生产化：消息去重 + @Agent 群聊路由 + 文件消息 + 断连重连 + access_token 轮换 | P0 | 3d | F7.2 |
| 15.2 | 企微应用回调验证：URL 验证 + 消息签名校验 + 加解密（AES-CBC）| P0 | 1d | F7.2 |
| 15.3 | 企微集成测试（私聊 + 群聊 + 文件 + 加解密 + 断连重连） | P0 | 1d | F7.2 |
...
#### Sprint 16: 飞书 Channel 生产就绪（W11-W12）

**目标**: 飞书 Channel 从原型提升到企业生产级别。

| # | 任务 | 优先级 | 预估 | 对应 Feature |
|---|------|--------|------|-------------|
| 16.1 | 飞书消息去重：基于 `message_id` + Redis-like 去重窗口（内存 LRU，1000 条滑动窗口），防止 webhook 重复推送 | P0 | 1d | F7.1 |
| 16.2 | 飞书群聊路由增强：@Agent 消息检测 + 线程跟踪（reply_in_thread）+ 群聊 Session Key 隔离 | P0 | 2d | F7.1 |
| 16.3 | 飞书文件/图片消息支持：接收文件 → 下载 → 传入 Agent（image 工具 / pdf 工具 / knowledge RAG） | P0 | 2d | F7.1 |
| 16.4 | 飞书卡片消息：Agent 回复支持飞书交互卡片（按钮、确认弹窗），用于权限确认和操作审批 | P1 | 2d | F7.1 |
| 16.5 | 飞书错误恢复：access_token 2h 自动轮换 + 断连自动重连（指数退避）+ 限频退避（20 次/秒上限） | P0 | 1d | F7.1 |
| 16.6 | 飞书集成测试（私聊 + 群聊 + 文件 + 断连重连 + 限频） | P0 | 1d | F7.1 |
