...
> **文档版本**: v6.3
> **创建日期**: 2026-03-20
> **文档状态**: 执行中（Sprint 11 ✅ 已完成, Sprint 12 ✅ 已完成, Sprint 13 ✅ 已完成, Sprint 14 ✅ 已完成, Sprint 15 待实施, Sprint 15.5 ✅ 已完成, Sprint 15.6 ✅ 已完成, Sprint 15.7 待实施）
...
#### Sprint 15: Sub-Agent & ReAct 追平 OpenClaw（新插入）
...
**验收标准**:
- [ ] 可配置 maxSpawnDepth=3 时支持 3 级嵌套
- [ ] 重试上限随 provider 数量动态调整
- [ ] ThinkLevel 渐进降级 high→medium→low→off
- [ ] compaction_start/end 事件正确转发到前端
- [ ] kill 父代时孙代也被终止
- [ ] 子代完成后父 session 自动收到结果（无需 yield_agents）
- [ ] session 模式子代可 resume 并保留上下文
...
#### Sprint 15.7: Heartbeat 对齐 OpenClaw（3 Phase 渐进实施）
...
**验收标准**:
- [ ] HEARTBEAT.md 空文件时跳过 LLM 调用，Token 消耗为零
- [ ] HEARTBEAT_OK Markdown/HTML 变体被正确检测（覆盖率 100%）
- [ ] 4 种 HeartbeatReason 使用不同 prompt，英文 prompt 遵从性测试通过
- [ ] 250ms 内多个唤醒请求合并为单次执行
- [ ] lightContext 模式 token < 2K
- [ ] contextKey 去重生效
- [ ] Cron 连续 5 次失败后自动禁用
- [ ] TaskRegistry /tasks API 返回正确的任务列表
- [ ] 全量测试通过，无回归
